...
def dbt_wh_pipeline():
    @task.bash(
        on_success_callback=dbt_task_callback, on_failure_callback=task_failure_callback
    )
...
def embedding_data_vector_db_group():
    ...
    def embedding_data_vector_db(data: list[dict]):
        embed_and_save_data_task(data)

    data = retrieve_data_from_lakehouse()
    embedding_data_vector_db(data)
