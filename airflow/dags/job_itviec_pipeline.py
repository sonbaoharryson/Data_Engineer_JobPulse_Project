...
def itviec_pipeline():
    ...
    def scrape_itviec_job(sources: dict):
        ...
        total_data_job_itviec = []
        for source, url in sources.items():
            logger.info(f"Processing source: {source} with URL: {url}")
            try:
                itviec_jobs = crawler.crawler(url)
                if itviec_jobs:
                    logger.info(f"Successfully scraped {len(itviec_jobs)} jobs from IT Viec")
            except Exception as e:
                logger.error(f"Error scraping IT Viec: {e}")
            total_data_job_itviec += itviec_jobs
        return total_data_job_itviec if total_data_job_itviec else None

    @task
    def insert_jobs(data):
        if not data:
            logger.info("No IT Viec jobs to insert")
            return
        insert_itviec_jobs(data)

    @task
    def post_job_to_discord():
        query_data = query_unposted_jobs(table_name='itviec_data_job')
        jobs, urls = query_data
        logger.info(f"Found {len(jobs)} new IT Viec jobs to post to Discord")
        if not jobs:
            logger.info('No new jobs from IT Viec to post to Discord')
            return
        try:
