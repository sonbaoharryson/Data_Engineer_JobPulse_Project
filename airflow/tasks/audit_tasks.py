...
class AuditLogger:
    ...
    def log_task_execution(
        ...
        try:
            ...
            execution_date = (
                dag_run.logical_date if dag_run else context.get("execution_date")
            )
            ...
            if task_instance:
                ...
                if "__" in task_instance_key:
                    ...
                    task_id = (
                        context.get("task", {}).task_id if context.get("task") else ""
                    )
                task_name = task_id
                task_group = None
            ...
            params = {
                ...
                "log_url": log_url,
                "exception_traceback": (
                    exception_traceback[:10000] if exception_traceback else None
                ),  # Limit size
...
def task_success_callback(context):
    ...
    if isinstance(return_value, dict):
        ...
        rows_scraped = (
            return_value.get("rows_scraped", 0)
            if not isinstance(return_value.get("rows_scraped", list), list)
            else len(return_value.get("rows_scraped"))
        )
...
def dbt_task_callback(context):
    ...
    audit_logger.log_task_execution(
        ...
        task_status="success",
        dbt_command=dbt_command,
        additional_metadata={"layer": layer, "task_group": "dbt_wh_pipeline"},
    )
...
def discord_task_callback(context):
    ...
    posts_sent = (
        return_value.get("posts_sent", 0) if isinstance(return_value, dict) else 0
    )
    posts_failed = (
        return_value.get("posts_failed", 0) if isinstance(return_value, dict) else 0
    )
    ...
    data_source = (
        "itviec"
        if "itviec" in task_id.lower()
        else "topcv" if "topcv" in task_id.lower() else None
    )
    ...
    audit_logger.log_task_execution(
        ...
        additional_metadata={"data_source": data_source},
    )
