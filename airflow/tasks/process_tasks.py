...
def load_crawl_sources_url(source_crawl: str):
    ...
    if source_crawl == "itviec":
        ...
        raise ValueError(
            f"Unknown source_crawl: {source_crawl}. Must be 'itviec' or 'topcv'"
        )
...
def upload_crawl_data_to_minio(
    ...
    try:
        ...
        logger.info(
            f"Uploaded {len(data)} {source_crawl} jobs to MinIO at {destination_file}"
        )
...
def get_data_from_minio(source_crawl: str, file_path: str):
    ...
    try:
        ...
        logger.info(
            f"Retrieved {len(data)} {source_crawl} jobs from MinIO at {file_path}"
        )
        return data
    except Exception as e:
        logger.error(
            f"Error retrieving {source_crawl} jobs from MinIO at {file_path}: {e}"
        )
        raise
...
def scrape_source_job(sources: dict, source_crawl: str):
    ...
    for source, url in sources.items():
        ...
        try:
            ...
            if dict_jobs:
                logger.info(
                    f"Successfully scraped {len(dict_jobs)} jobs from {source_crawl}"
                )
    ...
    if errors and not total_data_job:
        raise RuntimeError(
            f"Failed to scrape any jobs from {source_crawl}. Errors: {errors}"
        )

    deduped_jobs = deduplicate_jobs(total_data_job)

    source_expectations = (
        itviec_expectations if source_crawl == "itviec" else topcv_expectations
    )
    ...
    upload_file_path = upload_crawl_data_to_minio(
        data=deduped_jobs, source_crawl=source_crawl
    )
    return_dict = {
        "rows_processed": 0,
        "rows_inserted": 0,
...
def download_logos_and_upload_to_minio(data: list[dict]):
    ...
    for logo_item in data:
        ...
        logo_url = (
            logo_item.get("logo_url") if isinstance(logo_item, dict) else logo_item
        )
        try:
...
def post_job_to_discord(crawl_source: str):
    ...
    try:
        query_data = (
            query_unposted_jobs(table_name="itviec_data_job")
