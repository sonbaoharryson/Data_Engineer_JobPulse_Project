...
class TopCVScraper:
    ...
    def _extract_job_info(self, job) -> tuple:
        ...
        salary = _safe_text(
            _safe_find(job.find("label", class_="title-salary"), "span")
        ) or _safe_text(_safe_find(job.find("label", class_="salary"), "span"))
    ...
    def scrape_jobs(self, url: str) -> List[Dict[str, str]]:
        ...
        for idx, job in enumerate(jobs, 1):
            ...
            try:
                ...
                title, company, logo, job_url, location, salary, exp = (
                    self._extract_job_info(job)
                )
                data["title"] = title
